...
const SQL_EXPORT_PATH = path.join(process.cwd(), 'database.sql');
...
export async function getMySqlPool(forceRetry = false): Promise<mysql.Pool | null> {
  ...
  try {
    ...
    console.log(
      '✅ Connected to Aiven MySQL Database successfully at',
      `${MYSQL_CONFIG.host}:${MYSQL_CONFIG.port}/${MYSQL_CONFIG.database} (User: ${MYSQL_CONFIG.user}, SSL: Required)`
    );
    connection.release();
    ...
    isMySqlConnected = false;
    logDatabaseError(err, `Aiven MySQL Connection (${MYSQL_CONFIG.host}:${MYSQL_CONFIG.port}/${MYSQL_CONFIG.database})`);
    console.error(`[Aiven MySQL Error] Could not connect: ${err?.message || err}`);
  }
...
export function exportDatabaseSqlFile() {
  // Schema is stored and maintained directly in Aiven MySQL and database.sql
  if (!fs.existsSync(SQL_EXPORT_PATH)) {
