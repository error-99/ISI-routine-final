...
// Extract authenticated student directly from Aiven MySQL database via Bearer token
...
router.get('/health', async (req: Request, res: Response) => {
  ...
  res.json({
    ...
    db: status.connected
      ? `Aiven Cloud MySQL Connected (${status.host}:${status.port}/${status.database})`
      : `Aiven MySQL Disconnected (${status.lastError || 'Unreachable'})`,
    provider: 'Aiven Cloud',
    ssl: 'REQUIRED (TLS Encrypted)',
    engine: 'Aiven MySQL Exclusive (Direct Database Queries)',
    backendEnv: {
...
router.post('/db/retry', async (req: Request, res: Response) => {
  ...
  res.json({
    success: isConn,
    message: isConn ? 'Aiven Cloud MySQL connected!' : 'Connection failed. Check logs.',
  });
});
...
// -------------------------------------------------------------
// 2. PUBLIC METADATA ROUTES (QUERIED FROM AIVEN MYSQL)
// -------------------------------------------------------------

router.get('/departments', async (req: Request, res: Response) => {
  try {
    const pool = await getMySqlPool();
    if (!pool) throw new Error('Aiven MySQL pool not available');
...
router.post('/register', async (req: Request, res: Response) => {
  ...
  if (!pool) {
    return res.status(503).json({
      success: false,
      message: 'Aiven MySQL Database is currently unavailable.',
    });
  }
...
router.post('/login', async (req: Request, res: Response) => {
  ...
  if (!pool) {
    return res.status(503).json({
      success: false,
      message: 'Aiven MySQL Database is unavailable.',
    });
  }
...
// -------------------------------------------------------------
// 4. USER ACTIONS (DIRECT AIVEN MYSQL QUERIES)
// -------------------------------------------------------------
...
router.all('/user', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Available Courses for student's department from Aiven MySQL
    ...
    if (action === 'add_course') {
      ...
      const totalCredits = parseFloat(String(sumRows[0]?.total || 0));
      await pool.query('UPDATE students SET total_credits = ? WHERE student_id = ?', [
        totalCredits,
        student.student_id,
      ]);
    ...
    // Class Routines from Aiven MySQL
    if (action === 'routine') {
    ...
    // Class Tests (CTs) from Aiven MySQL
    if (action === 'ct') {
      if (student.enrolled_courses.length === 0) {
        return res.json({ success: true, data: { routine: [] } });
      }
    ...
    // Midterm & Final Exams from Aiven MySQL
    if (action === 'exams') {
      if (student.enrolled_courses.length === 0) {
        return res.json({ success: true, data: { routine: [] } });
      }
    ...
    // Notifications from Aiven MySQL
